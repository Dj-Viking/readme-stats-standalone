...
async function test2 () {
	try {
		const path = "/users/dj-viking/repos?per_page=141";
		const reposres = await fetch("https://api.github.com" + path);
		/**
		 * @type {RepoItem[]}
		 */
		const repoitems = await reposres.json();

		console.log("got repo stuff", repoitems.length);
		...
		.map((item) => {
			...
			return {
				p: new Promise(async resolve => {
					...
					resolve(JSON.stringify((await res.json())))
				}),
				url
			}
		});
		return new Promise(async resolve => {
			...
			fs.writeFileSync("langs", results.join("\n"), { encoding: "utf-8" });
